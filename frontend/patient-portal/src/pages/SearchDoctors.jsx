...
export default function SearchDoctors() {
  ...
  const filtered = doctors.filter(doc => {
    const name = doc.user ? (doc.user.firstName + ' ' + doc.user.lastName) : 'Unknown Doctor';
    const matchesSearch = name.toLowerCase().includes(search.toLowerCase()) || doc.specialization.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
      ...
      {loading ? (
        <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.4)', marginTop: '2rem' }}>Loading doctors...</p>
      ) : (
        /* Doctor Grid */
        <div style={{
          display: 'grid',
              ...
              <div key={doc.id} className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                  <div style={{
                    ...
                    <h3 style={{ margin: 0, fontFamily: 'Outfit, sans-serif' }}>{name}</h3>
                    <span style={{ fontSize: '0.9rem', color: '#ffb300' }}>★ {doc.rating || 'N/A'}</span>
                  </div>
                ...
                <Link 
                  to="/book-appointment" 
                  state={{ doctorId: doc.id, doctorName: name }} 
